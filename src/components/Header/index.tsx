...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header font-sans antialiased top-0 left-0 z-40 flex w-full items-center ${
          sticky
                    ...
                    {menuData.map((menuItem, index) => (
                      <li key={index} className="group relative">
                        {menuItem.path ? (
                          <Link
                            href={menuItem.path}
                            className={`flex py-2 text-[11px] font-bold uppercase tracking-widest lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${
                          ...
                          <>
                            <p
                              ...
                              className="text-dark group-hover:text-primary flex cursor-pointer items-center justify-between py-2 text-[11px] font-bold uppercase tracking-widest lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 dark:text-white/70 dark:group-hover:text-white"
                            >
                              {menuItem.title}
                              <span className="pl-1">
                                <svg width="15" height="14" viewBox="0 0 25 24">
                            ...
                            >
                              {menuItem.submenu && menuItem.submenu.map((submenuItem, subIndex) => (
                                <Link
                                  ...
                                  key={subIndex}
                                  className="text-dark hover:text-primary block rounded-sm py-2 text-[10px] font-bold uppercase tracking-widest lg:px-3 dark:text-white/70 dark:hover:text-white"
                                >
                                  {submenuItem.title}
                                </Link>
                              ))}
